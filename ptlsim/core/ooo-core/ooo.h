...
#define LSQ_SIZE (LDQ_SIZE + STQ_SIZE)
    ...
    struct PhysicalRegisterFile: public array<PhysicalRegister, MAX_PHYS_REG_FILE_SIZE> {
		...
		struct struct_cache{
			...
			int seu_buffer[SEU_BANDWIDTH];
			int seu_buffer_p;
			int seu_buffer_next[SEU_BANDWIDTH];
			int seu_buffer_pn;
		...
		int seu_register();
		int rfc_bus_bandwidth(){return (rf_cache_bus.request_on_the_fly + rf_cache.seu_buffer_pn);}
		int rfc_bus_saturated(){return (rfc_bus_bandwidth() >= RF_CACHE_BANDWIDTH);}
